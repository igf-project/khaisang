...
class CLS_CONTENTS{
	...
	public function getList($where=''){
		$sql="SELECT `tbl_contents`.* FROM `tbl_contents` ".$where;
		return $this->objmysql->Query($sql);
	}
    public function countContent($where=''){
        $objdata=new CLS_MYSQL;
        $sql="SELECT count(`tbl_contents`.`id`) as count FROM `tbl_contents` ".$where;
        $objdata->Query($sql);
	...
	public function getNameById($id){
		$objdata=new CLS_MYSQL;
		$sql="SELECT `title` FROM `tbl_contents` WHERE isactive=1 AND `id` = '$id'";
		$objdata->Query($sql);
		$row=$objdata->Fetch_Assoc();
		return $row['title'];
	}
    ...
    public function getListItem($strwhere="", $limit=""){
        $sql="SELECT `tbl_contents`.`id`, `tbl_contents`.`code`, `tbl_contents`.`thumb`, `tbl_contents`.`cdate`,
				`tbl_contents`.`title`, `tbl_contents`.`intro`, `tbl_contents`.`author`
			FROM `tbl_contents` ".$strwhere." ORDER BY `tbl_contents`.`id` DESC $limit";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
